...
import json
import numpy as np
from QASystem.model import Model
from QASystem.prepro import convert_to_features, word_tokenize
...
class Inference(object):
    def __init__(self):
        ...
        flags.DEFINE_integer("para_limit", 40000, "Limit length for paragraph")
            ...
            with tf.Session(config=sess_config) as sess:
                ...
                if self.query:
                    ...
                    fd = {'context:0': [c],
                        ...
                        'context_char:0': [ch],
                        'question_char:0': [qh]}
                    yp1,yp2,probs = sess.run([self.model.yp1, self.model.yp2, self.model.probs], feed_dict = fd)
                    score = np.max(np.outer(probs[0], probs[1]))
                    yp2[0] += 1
                    response = " ".join(context[yp1[0]:yp2[0]])
                    return (response, score)

    def test(self, passages, queries):
        ...
        with self.model.graph.as_default():
            ...
            with tf.Session(config=sess_config) as sess:
                ...
                if self.config.decay < 1.0:
                    sess.run(self.model.assign_vars)
                ans = {}
                for (passa, query) in zip(passages, queries):
                    self.query = (passa, query)
                    if self.query:
                        try:
                            context = word_tokenize(self.query[0].replace("''", '" ').replace("``", '" '))
                            c,ch,q,qh = convert_to_features(self.config, self.query, word_dictionary, char_dictionary)
