...
from RNet.func import cudnn_gru, native_gru, dot_attention, summ, ptr_net
from RNet.prepro import word_tokenize, convert_idx
...
# File path
target_dir = "RNet/data"
save_dir = "RNet/log/model"
...
class InfModel(object):
    ...
    def ready(self):
        ...
        with tf.variable_scope("predict"):
            ...
            self.probs = [tf.nn.log_softmax(logits1), tf.nn.log_softmax(logits2)]
...
class Inference(object):
    ...
    def response(self, context, question):
        ...
        span, context_idxs, ques_idxs, context_char_idxs, ques_char_idxs = self.prepro(
            context, question)
        yp1, yp2, probs= sess.run([model.yp1, model.yp2, model.probs], feed_dict={
        ...
        prob1 = probs[0].reshape(-1)
        prob2 = probs[1].reshape(-1)
        import torch
        scores = torch.ger(torch.Tensor(prob1), torch.Tensor(prob2))
        scores.triu_().tril_(prob1.shape[0] - 1)
        scores = scores.numpy()
        scores = scores.flatten()
        return context[start_idx: end_idx], max(scores)
